...
var InputGroup = /** @class */ (function () {
    ...
    InputGroup.prototype.componentDidLoad = function () {
        ...
        if (this.appendedButtons)
            try {
                r = JSON.parse(this.appendedButtons);
            }
            catch (e) {
                r = [], console.log("Error parsing the JSON string."), console.log(this.appendedButtons);
            }
        var n = [];
        if (this.prependedButtons)
            try {
                n = JSON.parse(this.prependedButtons);
            }
            catch (e) {
                n = [], console.log("Error parsing the JSON string."), console.log(this.prependedButtons);
            }
        return GD.Components.InputGroup({ appendedButtons: r, appendedLabel: this.appendedLabel, className: this.className, description: this.description, el: this.el.children[0], id: this.id, isLarge: this.isLarge, isPlainText: this.isPlainText, isReadonly: this.isReadonly, isSmall: this.isSmall, label: this.label, placeholder: this.placeholder, prependedButtons: n, prependedLabel: this.prependedLabel, rows: this.rows, type: this.type, value: this.value, onChange: function () {
                var t = [];
